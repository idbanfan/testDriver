#include <Windows.h>
#include <atlstr.h>
#include "Driver_Help.h"
#include <comdef.h>

const char* driver_error_info[2353] = { "-->0 操作成功完成。",\
"-->1 功能错误。 ",\
"-->2 系统找不到指定的文件。 ",\
"-->3 系统找不到指定的路径。 ",\
"-->4 系统无法打开文件。 ",\
...
"-->5773 该 DC 的 DNS 服务器不支持动态 DNS。 将文件 'SystemRoot,System32,Config,netlogon.dns' 中的 DNS 记录添加到伺服那个文件中引用的域的 DNS 服务器。 ",\
...
"-->10112 由于被拒绝，数据查询失败。" };
HANDLE h_DriverHandle = 0;
#define DriverName L"Name"
#define DriverLinks "\\\\.\\Test"
WCHAR DriverInstallPath[0xFF] = { L"C:\\CE_Driver.sys" };

BOOL DriverGetError(INT error, char* buffer) {
	char find[8] = { 0 };
	sprintf(find, "%d", error);
	for (ULONG i = 0; i < 2353; i++)
	{
		if (strstr(driver_error_info[i], find) != 0) {
			//找到
			memcpy(buffer, driver_error_info[i], strlen(driver_error_info[i]));
			return TRUE;
		}
		//OutputDebugStringA(driver_error_info[i]);
	}
	return FALSE;
}

INT DriverInstall(PCTSTR szServiceName, PCTSTR szDriverPath)
{
	HANDLE hDriverFile;
	...
	hDriverFile = CreateFile(szDriverPath,
		GENERIC_ALL,
		0, 
		NULL, 
		OPEN_EXISTING, 
	...
	if (hDriverFile == INVALID_HANDLE_VALUE)
	{
		...
		char err[1024] = { 0 };
		DriverGetError((int)GetLastError(), err);
		Out1(err);

		// DbgOut("CreateFile:%d", GetLastError());
		return GetLastError();
	}
	...
	{
		//DbgOut("OpenSCManager:%d", GetLastError());
		return GetLastError();
	}

	Out1("OpenSCManager之后");

	schService = CreateService(schSCManager, szServiceName, szServiceName,
		SERVICE_ALL_ACCESS, SERVICE_KERNEL_DRIVER, SERVICE_DEMAND_START,
		SERVICE_ERROR_NORMAL, szDriverPath, NULL, NULL, NULL, NULL, NULL);
	if (schService == NULL)
	{
		//DbgOut("CreateService:%d", GetLastError());
		CloseServiceHandle(schSCManager);
		return GetLastError();
	}

	Out1("CreateService之后");

	CloseServiceHandle(schService);

	Out1("CloseServiceHandle之后");
...
INT DriverStart(PCTSTR szServiceName)
{
	Out1("驱动启动准备");
	SC_HANDLE schSCManager;
	SC_HANDLE schService;
	...
	schService = OpenService(
		...
		szServiceName,            // 要打开的服务名
		SERVICE_ALL_ACCESS);      // 服务访问权限：所有权限
	if (schService == NULL)
	{
		//DbgOut("OpenService:%d", GetLastError());
		CloseServiceHandle(schSCManager);
		return GetLastError();
	}
	...
	{
		//DbgOut("StartService:%d", GetLastError());
		CloseServiceHandle(schService);
		CloseServiceHandle(schSCManager);
		return GetLastError();
	}
	...
	//UL_DeleteFile(deleteFilePath);

	Out1("驱动启动成功");
	return 0;
}

INT DriverStop(PCTSTR szServiceName)
{
	...
	{
		//DbgOut("ControlService:%d", GetLastError());
		CloseServiceHandle(schService);
		CloseServiceHandle(schSCManager);
		return GetLastError();
	}
...
INT DriverUninstall(PCTSTR szServiceName)
{
	// 打开服务控制管理器数据库
	SC_HANDLE schSCManager = OpenSCManager(
		NULL,                   // 目标计算机的名称,NULL：连接本地计算机上的服务控制管理器
	...
	SC_HANDLE schService = OpenService(schSCManager,           // 服务控件管理器数据库的句柄
		szServiceName,            // 要打开的服务名
		SERVICE_ALL_ACCESS);      // 服务访问权限：所有权限
	...
	{
		//DbgOut("DeleteService:%d", GetLastError());
		return GetLastError();
	}
	CloseServiceHandle(schService);
	CloseServiceHandle(schSCManager);
...
INT DriverLinks_(){
	h_DriverHandle = CreateFileA(DriverLinks, GENERIC_ALL, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_HIDDEN, NULL);
	if (h_DriverHandle == INVALID_HANDLE_VALUE)
		return GetLastError();
...
BOOL OnUnLoad() {
	Out1("驱动卸载准备");
	CloseHandle(h_DriverHandle);
	DriverStop(DriverName);
	DriverUninstall(DriverName);
	Out1("驱动卸载成功");
	return TRUE;
}
...
BOOL OnLoad() {
	...
	error = DriverInstall(DriverName, DriverInstallPath);
	error = DriverStart(DriverName);
	error = DriverLinks_();

	DriverGetError(error, err);
	Out2("DriverInstall，DriverStart，DriverLinks_产生的错误", err);
